...
import {
  buildMunicipalReportVariableCatalog,
  parseMunicipalReportTemplate,
  resolveMunicipalReportTemplate,
} from "@/utils/municipalReportTemplate";
...
describe("municipal report templates", () => {
  it("parses report and dynamic analysis markers", () => {
    const parsed = parseMunicipalReportTemplate(
      template(
        "[[report:introduction]]\nOlá\n[[analysis:indice_novo:situation]]\nTexto",
      ),
    );
    expect(parsed.errors).toEqual([]);
    expect(parsed.sections.map(({ key }) => key)).toEqual([
      "report:introduction",
      "analysis:indice_novo:situation",
    ]);
  });
  ...
  it("reports malformed markers and exposes a typed variable catalog", () => {
    expect(
      parseMunicipalReportTemplate(template("[[wrong]]\nTexto")).errors,
    ).toContain("Marcador inválido: [[wrong]]");
    expect(
      buildMunicipalReportVariableCatalog(report.templateVariables),
    ).toContainEqual({
      name: "percentual_seca",
      type: "percentage",
      description: "Variável do relatório municipal: percentual_seca.",
      example: 52,
    });
  });
});
