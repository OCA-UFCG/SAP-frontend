...
describe("municipal report value presentation", () => {
  it("keeps percentage presentation for territorial coverage indicators", () => {
    ...
    expect(formatMunicipalReportValue(42, analysis, "pt-BR")).toBe("42,00%");
...
describe("formatPercentage", () => {
  it("formats with pt-BR locale (comma decimal, dot thousands separator)", () => {
    expect(formatPercentage(1234.5, "pt-BR")).toBe("1.234,50");
    expect(formatPercentage(1234.5, "pt-BR", 2)).toBe("1.234,50");
    expect(formatPercentage(42, "pt-BR")).toBe("42,00");
    expect(formatPercentage(0.5, "pt-BR")).toBe("0,50");
  });

  it("formats with en-US locale (dot decimal, comma thousands separator)", () => {
    expect(formatPercentage(1234.5, "en-US")).toBe("1,234.50");
    expect(formatPercentage(1234.5, "en-US", 2)).toBe("1,234.50");
    expect(formatPercentage(42, "en-US")).toBe("42.00");
    expect(formatPercentage(0.5, "en-US")).toBe("0.50");
  });

  it("formats with es-AR locale (comma decimal, dot thousands separator)", () => {
    expect(formatPercentage(1234.5, "es-AR")).toBe("1.234,50");
    expect(formatPercentage(1234.5, "es-AR", 2)).toBe("1.234,50");
    expect(formatPercentage(42, "es-AR")).toBe("42,00");
    expect(formatPercentage(0.5, "es-AR")).toBe("0,50");
  });

  it("supports the locale identifiers used by the application routes", () => {
    expect(formatPercentage(42.5, "pt")).toBe("42,50");
    expect(formatPercentage(42.5, "en")).toBe("42.50");
    expect(formatPercentage(42.5, "es")).toBe("42,50");
  });
});
